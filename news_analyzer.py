...
from config import (
    ...
    NEWS_ARTICLE_MAX_LENGTH_FOR_GEMINI_SUMMARIZATION,  # Corrected import
    GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE  # Used for general prompt safety
)
...
class NewsAnalyzer:
    ...
    def analyze_single_news_item(self, news_event_db):
        ...
        if not sentiment_response.startswith("Error:"):
            try:
                ...
                if ":" in parts[0]:
                    ...
                    analysis_payload["sentiment"] = parts[0].strip().split(' ')[0]
        ...
        # General prompt safety truncation in GeminiAPIClient will apply if this is still too long
        ...
        news_analysis_entry = NewsEventAnalysis(
            news_event_id=news_event_db.id, analysis_date=current_analysis_time,
    ...
    def run_news_analysis_pipeline(self, category="general", count_to_fetch_from_api=MAX_NEWS_ARTICLES_PER_QUERY,
        ...
        for news_item_api_data in fetched_news_items_api:
            ...
            try:
                ...
                elif not news_event_db.full_article_text and latest_analysis:  # Needs re-analysis if full text was missing but now available
                    if news_event_db.full_article_text:
                        analysis_needed = True; logger.info(
                            f"News '{news_event_db.event_title[:50]}...' re-analyzing with newly scraped full text.")
                    else:
                        logger.info(
                            f"News '{news_event_db.event_title[:50]}...' already analyzed, full text still unavailable. Skipping.")
        ...
        if not instance_state.session or instance_state.session is not self.db_session:
            ...
            try:
                if not instance_state.has_identity and news_event_db_obj.id is None:
                    ...
                    if existing_in_session: news_event_db_obj = existing_in_session; logger.info(
                        f"Replaced transient NewsEvent for '{news_event_db_obj.source_url}' with instance from session."); return news_event_db_obj
                ...
                elif news_event_db_obj.source_url:
                    fallback_event = self.db_session.query(NewsEvent).filter_by(
                        source_url=news_event_db_obj.source_url).first()
                if not fallback_event: logger.critical(
                    f"CRITICAL: Failed to re-associate NewsEvent '{news_event_db_obj.event_title[:50]}...' with session."); return None
                logger.info(
