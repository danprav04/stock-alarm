...
import re
import json  # For handling JSON in DCF sensitivity
...
def calculate_cagr(end_value, start_value, years):
    ...
    if start_value == 0: return None  # Cannot divide by zero or calculate growth from zero base meaningfully
    # Handle negative start/end values more carefully
    if (start_value < 0 and end_value > 0) or (start_value > 0 and end_value < 0):
        return None  # Growth from negative to positive (or vice versa) is often not well-represented by CAGR
    if start_value < 0 and end_value < 0:  # Both negative, treat as absolute decline/growth
        # CAGR for negative numbers: ((end/start)^(1/N))-1 can be tricky if start > end (less negative).
        # Alternative: treat magnitudes if sensible for the metric. For financials, this is complex.
        # For simplicity and robustness, if both are negative, we might return None or a specific indicator.
        # Let's return None if start_value is negative as CAGR isn't standard here.
        return None
...
def get_value_from_statement_list(data_list, field, year_offset=0, report_date_for_log=None):
    """
    Safely gets a float value from a list of financial statement dictionaries.
    Assumes data_list is sorted with the most recent report at index 0.
    """
    if data_list and isinstance(data_list, list) and len(data_list) > year_offset:
        ...
        if report and isinstance(report, dict):
            ...
            return val
    # logger.debug(f"Could not retrieve field '{field}' from statement list (offset {year_offset}). List length: {len(data_list) if data_list else 'N/A'}")
    return None


def get_finnhub_concept_value(finnhub_quarterly_reports_data, report_section_key, concept_names_list, quarter_offset=0):
    """
    Extracts a specific financial concept value from Finnhub's financials-reported data.
    Assumes finnhub_quarterly_reports_data is sorted with most recent quarter at index 0.
    """
    if not finnhub_quarterly_reports_data or len(finnhub_quarterly_reports_data) <= quarter_offset: return None
    report_data = finnhub_quarterly_reports_data[quarter_offset]  # Most recent is at index 0
    ...
    for item in section_items:
        # Finnhub uses 'concept' (e.g., "us-gaap:Revenues") and 'label' (e.g., "Revenues")
        # Check both if the concept matches any in the provided list.
...
def get_alphavantage_value(av_quarterly_reports, field_name, quarter_offset_from_latest=0):
    """
    Extracts a value from Alpha Vantage quarterly reports.
    Assumes av_quarterly_reports are sorted with most recent first (after potential reversal in API client).
    """
    if not av_quarterly_reports or len(av_quarterly_reports) <= quarter_offset_from_latest:
        return None
    # AV reports are reversed in client to be most recent first.
...
def get_fmp_value(fmp_quarterly_reports, field_name, quarter_offset_from_latest=0):
    """
    Extracts a value from FMP quarterly reports.
    Assumes fmp_quarterly_reports are sorted with most recent first.
    """
    if not fmp_quarterly_reports or len(fmp_quarterly_reports) <= quarter_offset_from_latest:
        return None
...
class StockAnalyzer:
    def __init__(self, ticker):
        ...
        self.db_session = next(get_db_session())  # Each analyzer instance gets its own session
        self.stock_db_entry = None
        self._financial_data_cache = {}  # Cache for data fetched during the analysis of this stock
        self.data_quality_warnings = []  # To track issues for AI confidence adjustment
        ...
        try:
            ...
            self._close_session_if_active()  # Ensure session is closed on init failure
            # Raise a specific error that can be caught by the caller in main.py
            raise RuntimeError(
    ...
    def _get_or_create_stock_entry(self):
        if not self.db_session.is_active:  # Should not happen if init was successful, but defensive
            logger.warning(f"Session for {self.ticker} inactive in _get_or_create. Re-establishing.")
            self._close_session_if_active()  # Close any lingering old one
        ...
        if profile_fmp_list and isinstance(profile_fmp_list, list) and len(profile_fmp_list) > 0 and profile_fmp_list[
            ...
            cik = data.get('cik')  # FMP CIKs are often integers, SEC EDGAR needs 10-digit string
            ...
            if profile_fh:
                ...
                # Finnhub doesn't directly provide sector or CIK in profile2
                ...
                time.sleep(2)  # AV is slower sometimes
                if overview_av and overview_av.get("Symbol") == self.ticker:  # Check if data is for the correct ticker
                    ...
                    cik = overview_av.get('CIK')  # AV CIKs are usually strings, sometimes need padding
                    if cik: cik = str(cik).zfill(10)
        ...
        if not cik and self.ticker:
            ...
            time.sleep(0.5)  # SEC EDGAR can be sensitive
            if cik_from_edgar:
        ...
        if not self.stock_db_entry:
            ...
            try:
                ...
                self.db_session.refresh(self.stock_db_entry)  # Get ID and any defaults
            except SQLAlchemyError as e:
                self.db_session.rollback()
                logger.error(f"Error creating stock entry for {self.ticker}: {e}", exc_info=True)
                raise  # Re-raise to halt initialization
        else:  # Stock entry exists, update if new info is better
            updated = False
            ...
            if sector and self.stock_db_entry.sector != sector:
                ...
                updated = True
            if cik and self.stock_db_entry.cik != cik:  # If new CIK is found and different
                self.stock_db_entry.cik = cik;
                updated = True
            elif not self.stock_db_entry.cik and cik:  # If DB CIK was null and now we have one
                self.stock_db_entry.cik = cik;
                updated = True
    ...
    def _fetch_financial_statements(self):
        ...
        statements_cache = {
            ...
            "fmp_income_quarterly": [],  # Added for quarterly revenue cross-check
            "finnhub_financials_quarterly_reported": {"data": []},  # Finnhub provides 'data' list
            "alphavantage_income_quarterly": {"quarterlyReports": []},  # AV has specific structure
        ...
        try:
            ...
            # FMP Quarterly (primarily for revenue cross-check)
            statements_cache["fmp_income_quarterly"] = self.fmp.get_financial_statements(self.ticker,
                                                                                         "income-statement", "quarter",
                                                                                         8) or [];
            time.sleep(1.5)  # Fetch last 8 quarters
            logger.info(
                f"FMP Quarterly Income for {self.ticker}: {len(statements_cache['fmp_income_quarterly'])} records.")

            # Finnhub Quarterly Reported Financials
            fh_q_data = self.finnhub.get_financials_reported(self.ticker, freq="quarterly",
                                                             count=8)  # Fetch last 8 quarters
            time.sleep(1.5)
            if fh_q_data and isinstance(fh_q_data, dict) and fh_q_data.get("data"):
                statements_cache["finnhub_financials_quarterly_reported"] = fh_q_data  # Finnhub data is newest first
            ...
            time.sleep(15)  # AV rate limits are strict
            if av_income_q and isinstance(av_income_q, dict) and av_income_q.get("quarterlyReports"):
                statements_cache["alphavantage_income_quarterly"] = av_income_q  # AV client reverses to newest first
                logger.info(
    ...
    def _fetch_key_metrics_and_profile_data(self):
        ...
        time.sleep(1.5)
        key_metrics_quarterly_fmp = self.fmp.get_key_metrics(self.ticker, "quarterly", 8)  # Fetch 8 quarters
        time.sleep(1.5)
        if key_metrics_quarterly_fmp is None:
            logger.warning(f"FMP quarterly key metrics API call failed for {self.ticker}. Data will be empty list.");
            self._financial_data_cache['key_metrics_quarterly_fmp'] = []
        else:  # FMP returns list or empty list if successful but no data
        ...
        # Ensure FMP profile is fetched if not already in cache (e.g. if init failed on FMP but succeeded on another source)
    ...
    def _calculate_valuation_ratios(self, latest_km_q_fmp, latest_km_a_fmp, basic_fin_fh_metric):
        ratios = {}
        # Prioritize TTM from FMP quarterly, then FMP annual, then Finnhub
        ...
        ratios["pb_ratio"] = safe_get_float(latest_km_q_fmp, "priceToBookRatioTTM") or \
                             ...
                             safe_get_float(basic_fin_fh_metric, "pbAnnual")  # Finnhub uses pbAnnual
        ...
        # Finnhub doesn't have direct EV/Sales in basic financials

        ratios["ev_to_ebitda"] = safe_get_float(latest_km_q_fmp, "evToEbitdaTTM") or \
                                 safe_get_float(latest_km_a_fmp, "evToEbitda")
        # Finnhub doesn't have direct EV/EBITDA in basic financials

        # Dividend Yield: FMP TTM > FMP Annual > Finnhub Annual (Finnhub reports as %, FMP as decimal)
        ...
        div_yield_fh_raw = safe_get_float(basic_fin_fh_metric, "dividendYieldAnnual")  # This is a percentage
    ...
    def _calculate_profitability_metrics(self, income_annual_fmp, balance_annual_fmp, latest_km_a_fmp):
        metrics = {}
        if income_annual_fmp:  # List of annual income statements, newest first
            latest_ia = income_annual_fmp[0]  # Most recent annual report
            # EPS: FMP annual statement > FMP key metrics annual
            ...
            metrics["operating_profit_margin"] = safe_get_float(latest_ia, "operatingIncomeRatio")  # FMP name

            # Interest Coverage Ratio = EBIT / Interest Expense
            ebit = safe_get_float(latest_ia, "operatingIncome")  # Operating Income is often used as EBIT
            interest_expense = safe_get_float(latest_ia, "interestExpense")
            if ebit is not None and interest_expense is not None and abs(
                    interest_expense) > 1e-6:  # Avoid division by zero or tiny interest
                metrics["interest_coverage_ratio"] = ebit / abs(
                    interest_expense)  # Use absolute for interest expense magnitude

        if balance_annual_fmp and income_annual_fmp:  # Need both for ROE, ROA, ROIC
            latest_ba = balance_annual_fmp[0]  # Most recent annual balance sheet
            ...
            # Invested Capital = Total Debt + Total Equity - Cash & Cash Equivalents - Short Term Investments (sometimes)
            ...
            effective_tax_rate = 0.21  # Default statutory rate if not calculable
            if income_tax_expense_roic is not None and income_before_tax_roic is not None and income_before_tax_roic != 0:
                ...
                # Ensure tax rate is reasonable (e.g., 0% to 50%)
                if 0 <= calculated_tax_rate <= 0.50:
                    effective_tax_rate = calculated_tax_rate
                else:
                    logger.debug(
                        f"Calculated tax rate {calculated_tax_rate:.2%} for {self.ticker} is unusual. Using default {effective_tax_rate:.2%}.")
            ...
            # total_equity already fetched
            cash_equivalents_roic = get_value_from_statement_list(balance_annual_fmp, "cashAndCashEquivalents", 0) or 0
            # Short-term investments can be debated. For simplicity, often excluded or only "non-operating" cash is subtracted.
            # Let's use a common definition: (Total Debt + Total Equity - Cash & Equivalents)
    ...
    def _calculate_financial_health_metrics(self, balance_annual_fmp, income_annual_fmp, latest_km_a_fmp):
        ...
        if balance_annual_fmp:
            ...
            # Debt-to-Equity: FMP Key Metrics > Calculated from Balance Sheet
            metrics["debt_to_equity"] = safe_get_float(latest_km_a_fmp, "debtToEquity")
            if metrics["debt_to_equity"] is None:  # Fallback calculation
            ...
            # Quick Ratio = (Cash & Equivalents + Short Term Investments + Net Receivables) / Current Liabilities
            cash_equivalents = safe_get_float(latest_ba, "cashAndCashEquivalents", 0)  # Default to 0 if missing
        ...
        # Debt-to-EBITDA: Calculated using EBITDA from FMP Key Metrics or Income Statement
        latest_annual_ebitda_km = safe_get_float(latest_km_a_fmp, "ebitda")
        latest_annual_ebitda_is = get_value_from_statement_list(income_annual_fmp, "ebitda", 0)
    ...
    def _get_cross_validated_quarterly_revenue(self, statements_cache):
        """
        Tries to get the most reliable recent quarterly revenue from multiple sources.
        Performs a sanity check against previous quarters.
        Returns (latest_q_revenue, previous_q_revenue, source_name, historical_avg_q_revenue)
        """
        latest_q_revenue, previous_q_revenue, source_name = None, None, None
        historical_revenues = []  # For sanity check

        # Define revenue field names for different sources
        revenue_fields = {
        ...
        for src_key in PRIORITY_REVENUE_SOURCES:  # Defined in config
            try:
                if src_key == "fmp_quarterly" and statements_cache.get('fmp_income_quarterly'):
                    reports = statements_cache['fmp_income_quarterly']  # Newest first
                    ...
                    if latest_val is not None:
                        ...
                        for i in range(min(len(reports), 5)):  # Get up to 5 historical points for avg
                            hist_rev = get_fmp_value(reports, revenue_fields[src_key], i)
                            if hist_rev is not None: historical_revenues.append(hist_rev)
                        break
                elif src_key == "alphavantage_quarterly" and statements_cache.get('alphavantage_income_quarterly',
                                                                                  {}).get('quarterlyReports'):
                    reports = statements_cache['alphavantage_income_quarterly']['quarterlyReports']  # Newest first
                    if not reports: continue
                    ...
                    if latest_val is not None:
                        ...
                        for i in range(min(len(reports), 5)):
                            hist_rev = get_alphavantage_value(reports, revenue_fields[src_key], i)
                            if hist_rev is not None: historical_revenues.append(hist_rev)
                        break
                elif src_key == "finnhub_quarterly" and statements_cache.get('finnhub_financials_quarterly_reported',
                                                                             {}).get('data'):
                    reports = statements_cache['finnhub_financials_quarterly_reported']['data']  # Newest first
                    if not reports: continue
                    ...
                    if latest_val is not None:
                        ...
                        for i in range(min(len(reports), 5)):
                            hist_rev = get_finnhub_concept_value(reports, 'ic', revenue_fields[src_key], i)
                            if hist_rev is not None: historical_revenues.append(hist_rev)
                        break
            except Exception as e:
                logger.warning(f"Error processing quarterly revenue from {src_key} for {self.ticker}: {e}")
                continue

        avg_historical_q_revenue = None
        if historical_revenues:
            # Use the most recent few historical points (excluding current latest if it's part of historical_revenues)
            # If latest_q_revenue is in historical_revenues[0], use historical_revenues[1:] for average
            points_for_avg = [r for r in historical_revenues if r is not None and r > 0]  # Exclude zero/None

            # Ensure we are averaging previous quarters, not including the current latest one in the average base
            avg_base_points = []
            if points_for_avg:
                if points_for_avg[0] == latest_q_revenue and len(points_for_avg) > 1:  # if latest is the first item
                    avg_base_points = points_for_avg[1:]
                else:  # if latest_q_revenue is not the first or not present (e.g. from different source)
                    avg_base_points = points_for_avg

            if len(avg_base_points) > 1:  # Need at least 2 previous points for a meaningful average
                ...
                if latest_q_revenue is not None and avg_historical_q_revenue > 0:
                    ...
                    if deviation > Q_REVENUE_SANITY_CHECK_DEVIATION_THRESHOLD:
                        warning_msg = (
                            ...
                            f"deviates by {deviation:.2%} from avg of recent historical quarters ({avg_historical_q_revenue:,.0f}). Review data accuracy.")
                        logger.warning(warning_msg)
                        self.data_quality_warnings.append(warning_msg)
        else:
            logger.info(f"Not enough historical quarterly revenue data to perform sanity check for {self.ticker}.")

        if latest_q_revenue is None:
            logger.error(f"Could not determine latest quarterly revenue for {self.ticker} from any source.")
            self.data_quality_warnings.append("CRITICAL: Latest quarterly revenue could not be determined.")
        else:
            logger.info(
                f"Using latest quarterly revenue: {latest_q_revenue:,.0f} (Source: {source_name}) for {self.ticker}.")
    ...
    def _calculate_growth_metrics(self, income_annual_fmp, statements_cache):
        metrics = {"key_metrics_snapshot": {}}  # Snapshot for specific data points

        # YoY Growth (from FMP Annual Income Statements)
        ...
        # CAGR Growth (from FMP Annual Income Statements)
        if len(income_annual_fmp) >= 3:  # Need at least 3 years for 2-year CAGR (end vs start of 2-yr period)
            metrics["revenue_growth_cagr_3yr"] = calculate_cagr(
                get_value_from_statement_list(income_annual_fmp, "revenue", 0),  # Current year
                get_value_from_statement_list(income_annual_fmp, "revenue", 2),  # 2 years ago (for 3 data points total)
                2  # Number of years in period
            )
        ...
        if len(income_annual_fmp) >= 5:  # Need 5 years for 4-year CAGR
        ...
        # QoQ Revenue Growth (Cross-validated)
        ...
        if latest_q_rev is not None:
            ...
            if prev_q_rev is not None:
                ...
                logger.info(
                    f"Previous quarter revenue not available from source {rev_src_name} for {self.ticker}. Cannot calculate QoQ revenue growth.")
    ...
    def _calculate_cash_flow_and_trend_metrics(self, cashflow_annual_fmp, balance_annual_fmp, profile_fmp):
        ...
        if cashflow_annual_fmp:
            ...
            # Shares outstanding: FMP profile > (Market Cap / Price)
            ...
            if fcf_latest_annual is not None and shares_outstanding and shares_outstanding != 0:
                ...
                if mkt_cap_profile and mkt_cap_profile != 0:  # Use direct market cap for FCF Yield
                    metrics["free_cash_flow_yield"] = fcf_latest_annual / mkt_cap_profile

            # FCF Trend (based on last 3 annual FCFs)
            if len(cashflow_annual_fmp) >= 3:
                fcf0 = get_value_from_statement_list(cashflow_annual_fmp, "freeCashFlow", 0)  # Most recent
                fcf1 = get_value_from_statement_list(cashflow_annual_fmp, "freeCashFlow", 1)
                fcf2 = get_value_from_statement_list(cashflow_annual_fmp, "freeCashFlow", 2)  # Oldest of the three
    ...
    def _calculate_derived_metrics(self):
        ...
        # Get data from cache (fetched earlier)
        statements = self._financial_data_cache.get('financial_statements', {})
        # Ensure lists are sorted with most recent data first (index 0)
        # FMP data usually comes newest first. AV data is reversed in client. Finnhub newest first.
        ...
        # Get latest records from potentially empty lists safely
        ...
        all_metrics.update(
            self._calculate_profitability_metrics(income_annual_fmp, balance_annual_fmp, latest_km_a_fmp))
        all_metrics.update(
            self._calculate_financial_health_metrics(balance_annual_fmp, income_annual_fmp, latest_km_a_fmp))

        growth_metrics_result = self._calculate_growth_metrics(income_annual_fmp,
                                                               statements)  # Pass full statements_cache
        all_metrics.update(growth_metrics_result)  # This includes its own key_metrics_snapshot

        all_metrics.update(
            self._calculate_cash_flow_and_trend_metrics(cashflow_annual_fmp, balance_annual_fmp, profile_fmp))

        # Final cleanup of metrics: ensure None for NaN/Inf
        final_metrics = {}
        for k, v in all_metrics.items():
            if k == "key_metrics_snapshot":  # Handle nested dict separately
                ...
                final_metrics[k] = None  # Explicitly set to None if original was None

        log_metrics = {k: v for k, v in final_metrics.items() if k != "key_metrics_snapshot"}  # For cleaner logging
    ...
    def _perform_dcf_analysis(self):
        ...
        dcf_results = {
            ...
            "dcf_assumptions": {
                ...
                "start_fcf": None,
                "start_fcf_basis": "N/A",  # e.g., "LTM FCF", "Latest Annual FCF"
                "fcf_growth_rates_projection": [],
                "initial_fcf_growth_rate_used": None,
                "initial_fcf_growth_rate_basis": "N/A",  # e.g., "Historical 3yr FCF CAGR", "Revenue Growth Proxy"
                "sensitivity_analysis": []  # To store sensitivity results
            }
        }
        assumptions = dcf_results["dcf_assumptions"]  # Shorthand

        cashflow_annual_fmp = self._financial_data_cache.get('financial_statements', {}).get('fmp_cashflow_annual', [])
        profile_fmp = self._financial_data_cache.get('profile_fmp', {})
        ...
        # Start FCF: Use latest annual FCF
        current_fcf_annual = get_value_from_statement_list(cashflow_annual_fmp, "freeCashFlow", 0)
        if current_fcf_annual is None or current_fcf_annual <= 10000:  # Arbitrary small threshold
            logger.warning(
                f"Current annual FCF for {self.ticker} is {current_fcf_annual}. DCF requires substantial positive FCF. Skipping DCF.")
            return dcf_results
        assumptions["start_fcf"] = current_fcf_annual
        assumptions[
            "start_fcf_basis"] = f"Latest Annual FCF ({cashflow_annual_fmp[0].get('date') if cashflow_annual_fmp else 'N/A'})"

        # Initial FCF Growth Rate Estimation
        fcf_growth_rate_3yr_cagr = None
        if len(cashflow_annual_fmp) >= 4:  # Need 4 data points for 3-year CAGR
            fcf_start_for_cagr = get_value_from_statement_list(cashflow_annual_fmp, "freeCashFlow", 3)  # 3 years ago
            if fcf_start_for_cagr and fcf_start_for_cagr > 0:  # Avoid issues with negative or zero base
                fcf_growth_rate_3yr_cagr = calculate_cagr(current_fcf_annual, fcf_start_for_cagr, 3)

        initial_fcf_growth_rate = DEFAULT_PERPETUAL_GROWTH_RATE  # Default to perpetual if no better estimate
        assumptions["initial_fcf_growth_rate_basis"] = "Default (Perpetual Growth Rate)"

        # Prioritize growth estimates: Historical FCF CAGR > Revenue CAGR > YoY Revenue Growth > Default
        ...
        if not isinstance(initial_fcf_growth_rate,
                          (int, float)): initial_fcf_growth_rate = DEFAULT_PERPETUAL_GROWTH_RATE  # Fallback
        initial_fcf_growth_rate = min(max(initial_fcf_growth_rate, -0.05),
                                      0.15)  # Cap/floor growth rate to reasonable bounds for projection
        assumptions["initial_fcf_growth_rate_used"] = initial_fcf_growth_rate

        # --- Core DCF Calculation Function (to allow for sensitivity) ---
        def calculate_dcf_value(start_fcf, initial_growth, discount_rate, perpetual_growth, proj_years, shares):
            ...
            growth_rate_decline_per_year = (initial_growth - perpetual_growth) / float(
                proj_years) if proj_years > 0 else 0
            ...
            for i in range(proj_years):
                ...
                current_year_growth_rates.append(round(current_year_growth_rate, 4))

            if not projected_fcfs: return None, []  # Should not happen if proj_years > 0

            # Terminal Value
            terminal_year_fcf_for_tv = projected_fcfs[-1] * (1 + perpetual_growth)
            terminal_value_denominator = discount_rate - perpetual_growth
            if terminal_value_denominator <= 1e-6:  # Avoid division by zero or very small denominator
                logger.warning(
                    f"DCF for {self.ticker}: Discount rate ({discount_rate}) near/below perpetual growth ({perpetual_growth}). Terminal Value unreliable.")
                terminal_value = 0  # Or handle as error
            else:
            ...
            if shares != 0:
                return intrinsic_equity_value / shares, current_year_growth_rates
            return None, current_year_growth_rates

        # --- End Core DCF Calculation Function ---

        # Base Case Calculation
        base_iv_per_share, base_fcf_growth_rates = calculate_dcf_value(
            assumptions["start_fcf"], assumptions["initial_fcf_growth_rate_used"],
        ...
        if base_iv_per_share is not None:
            ...
            logger.error(f"DCF base case calculation failed for {self.ticker}.")
            return dcf_results  # Return early if base case fails

        # Sensitivity Analysis
        sensitivity_scenarios = [
            {"dr_adj": -0.005, "pgr_adj": 0.0, "label": "Discount Rate -0.5%"},  # DR 8.5%
            {"dr_adj": +0.005, "pgr_adj": 0.0, "label": "Discount Rate +0.5%"},  # DR 9.5%
            {"dr_adj": 0.0, "pgr_adj": -0.0025, "label": "Perp. Growth -0.25%"},  # PGR 2.25%
            {"dr_adj": 0.0, "pgr_adj": +0.0025, "label": "Perp. Growth +0.25%"}  # PGR 2.75%
        ]
        ...
        for scenario in sensitivity_scenarios:
            ...
            # Ensure pgr < dr for sensitivity
            if sens_pgr >= sens_dr - 0.001:  # Maintain a small spread
                logger.debug(
                    f"Skipping DCF sensitivity scenario '{scenario['label']}' for {self.ticker} as PGR ({sens_pgr:.3f}) >= DR ({sens_dr:.3f}).")
                continue

            iv_sens, _ = calculate_dcf_value(
                assumptions["start_fcf"], assumptions["initial_fcf_growth_rate_used"],
                sens_dr, sens_pgr,
                assumptions["projection_years"], shares_outstanding
            )
            if iv_sens is not None:
                upside_sens = (
                                          iv_sens - current_price) / current_price if current_price and current_price != 0 else None
        ...
        logger.info(
            f"DCF for {self.ticker}: Base IV/Share: {dcf_results.get('dcf_intrinsic_value', 'N/A'):.2f}, "
    ...
    def _summarize_text_chunked(self, text_to_summarize, base_context, section_specific_instruction,
        ...
        """Summarizes text, using chunking for very long inputs based on config."""
        ...
        logger.info(
            f"Summarizing '{base_context}' for {company_name_ticker_prompt}, original length: {text_len} chars.")

        # Use SUMMARIZATION_MAX_CONCAT_SUMMARIES_CHARS as the threshold for a single pass.
        # This config value should be set to a reasonable limit for one LLM call.
        ...
        while start < text_len:
            ...
            start = end - SUMMARIZATION_CHUNK_OVERLAP_CHARS if end < text_len and SUMMARIZATION_CHUNK_OVERLAP_CHARS < SUMMARIZATION_CHUNK_SIZE_CHARS else end

        chunk_summaries = []
        for i, chunk in enumerate(chunks):
            logger.info(
                f"Summarizing chunk {i + 1}/{len(chunks)} for '{base_context}' of {company_name_ticker_prompt} (length: {len(chunk)} chars).")
            chunk_context = f"This is chunk {i + 1} of {len(chunks)} from the '{base_context}' section for {company_name_ticker_prompt}."
            chunk_instruction = f"Summarize this chunk. Focus on key facts and figures relevant to: {section_specific_instruction}"
            summary = self.gemini.summarize_text_with_context(chunk, chunk_context, chunk_instruction)
            time.sleep(2)  # API courtesy
            if summary and not summary.startswith("Error:"):
                chunk_summaries.append(summary)
            else:
                chunk_summaries.append(f"[AI error or no content for chunk {i + 1} of '{base_context}']")
        if not chunk_summaries:  # Should not happen if chunks were created
        ...
        # If concatenated summaries are too long for a final pass (as defined by SUMMARIZATION_MAX_CONCAT_SUMMARIES_CHARS)
        if len(concatenated_summaries) > SUMMARIZATION_MAX_CONCAT_SUMMARIES_CHARS:
            ...
            final_summary_instruction = f"Synthesize these individual chunk summaries into a single, cohesive overview of the '{base_context}', maintaining factual accuracy and addressing the original goal: {section_specific_instruction}."
        ...
        else:  # Concatenated summaries are short enough
    ...
    def _fetch_and_summarize_10k(self):
        ...
        summary_results = {"qualitative_sources_summary": {}}  # Store lengths and URLs here
        ...
        time.sleep(0.5)
        if not filing_url:
            logger.info(f"No recent 10-K found for {self.ticker}, trying 10-K/A.")
        ...
        text_content = self.sec_edgar.get_filing_text(filing_url)  # Uses APIClient's request method
        if not text_content:
        ...
        # TEN_K_KEY_SECTIONS is S1_KEY_SECTIONS from config
        ...
        # Define sections and specific instructions for summarization
        section_details = {
        ...
        for section_key, (prompt_section_name, specific_instruction) in section_details.items():
            ...
            summary, source_len = self._summarize_text_chunked(
                section_text,
                prompt_section_name,  # Base context for the chunker
                specific_instruction,  # Specific instruction for Gemini
        ...
        # --- Derived Qualitative Summaries using AI ---
        biz_summary_str = summary_results.get("business_summary", "")
        if biz_summary_str.startswith("Section not found") or biz_summary_str.startswith(
            "AI summary error"): biz_summary_str = ""

        mda_summary_str = summary_results.get("mda_summary", "")
        if mda_summary_str.startswith("Section not found") or mda_summary_str.startswith(
            "AI summary error"): mda_summary_str = ""

        risk_summary_str = summary_results.get("risk_factors_summary", "")
        if risk_summary_str.startswith("Section not found") or risk_summary_str.startswith(
            "AI summary error"): risk_summary_str = ""

        # Economic Moat Analysis
        moat_input_text = (f"Business Summary:\n{biz_summary_str}\n\nRisk Factors Summary:\n{risk_summary_str}").strip()
        if moat_input_text and (biz_summary_str or risk_summary_str):  # Only proceed if some relevant text exists
            moat_prompt = (
                ...
                f"for {company_name_for_prompt} ({self.ticker}), based on the following summaries from its 10-K:\n\n{moat_input_text}\n\n"
                f"Provide a concise analysis of its key economic moats. For each identified moat, briefly explain the evidence from the text and assess its perceived strength (e.g., Very Strong, Strong, Moderate, Weak). "
                f"If certain moats are not strongly evident, state that.")
            ...
            summary_results[
                "economic_moat_summary"] = "Insufficient input from 10-K summaries for economic moat analysis."

        # Industry Trends & Positioning Analysis
        industry_context_text = (f"Company: {company_name_for_prompt} ({self.ticker})\n"
                                 f"Industry: {self.stock_db_entry.industry or 'Not Specified'}\n"
                                 f"Sector: {self.stock_db_entry.sector or 'Not Specified'}\n\n"
                                 f"Business Summary (from 10-K):\n{biz_summary_str}\n\n"
                                 f"MD&A Highlights (from 10-K):\n{mda_summary_str}").strip()
        if biz_summary_str:  # Business summary is key for this
            industry_prompt = (
                f"Based on the provided information for {company_name_for_prompt} ({self.ticker}):\n\n{industry_context_text}\n\n"
        ...
        # Rename mda_summary to management_assessment_summary for consistency with model/email
        if "mda_summary" in summary_results:
            ...
            if "mda_10k_source_length" in summary_results["qualitative_sources_summary"]:
                summary_results["qualitative_sources_summary"]["management_assessment_10k_source_length"] = \
                summary_results["qualitative_sources_summary"].pop("mda_10k_source_length")
    ...
    def _fetch_and_analyze_competitors(self):
        ...
        peers_data_finnhub = self.finnhub.get_company_peers(self.ticker)  # Returns a list of peer tickers
        time.sleep(1.5)
        ...
        if not peers_data_finnhub or not isinstance(peers_data_finnhub, list) or not peers_data_finnhub[0]:
            ...
            self._financial_data_cache['competitor_analysis'] = {
                "summary": "No peer data found from primary source (Finnhub).", "peers_data": []}
            return "No peer data found from primary source (Finnhub)."

        # Finnhub peers sometimes has the current ticker as the first element.
        # It can also be an empty list within a list: [['']] if no peers.
        if isinstance(peers_data_finnhub[0], list):  # Handle [['', 'PEER1', ...]]
            peers_data_finnhub = peers_data_finnhub[0]

        peer_tickers = [p for p in peers_data_finnhub if p and p.upper() != self.ticker.upper()][
        ...
        for peer_ticker in peer_tickers:
            try:
                ...
                peer_profile_fmp_list = self.fmp.get_company_profile(peer_ticker);
                time.sleep(1.5)
                peer_profile_fmp = peer_profile_fmp_list[0] if peer_profile_fmp_list and isinstance(
                    peer_profile_fmp_list, list) and peer_profile_fmp_list[0] else {}

                peer_metrics_fmp_list = self.fmp.get_key_metrics(peer_ticker, period="annual", limit=1);
                time.sleep(1.5)
                peer_metrics_fmp = peer_metrics_fmp_list[0] if peer_metrics_fmp_list and isinstance(
                    peer_metrics_fmp_list, list) and peer_metrics_fmp_list[0] else {}

                peer_fh_basics = {}
                if not peer_metrics_fmp.get("peRatio") or not peer_metrics_fmp.get(
                        "priceSalesRatio"):  # Check if FMP provided key metrics
                    peer_fh_basics_data = self.finnhub.get_basic_financials(peer_ticker);
                    time.sleep(1.5)
                    peer_fh_basics = peer_fh_basics_data.get("metric", {}) if peer_fh_basics_data else {}

                peer_name = peer_profile_fmp.get("companyName", peer_ticker)
                ...
                ps_ratio = safe_get_float(peer_metrics_fmp, "priceSalesRatio") or safe_get_float(peer_fh_basics,
                                                                                                 "psTTM")

                peer_info = {
                    "ticker": peer_ticker, "name": peer_name,
                    "market_cap": market_cap, "pe_ratio": pe_ratio, "ps_ratio": ps_ratio
                }
                if peer_name != peer_ticker or market_cap or pe_ratio or ps_ratio:  # Add if any meaningful data was found
                    peer_details_list.append(peer_info)
            except Exception as e:
                logger.warning(f"Error fetching data for peer {peer_ticker}: {e}", exc_info=True)
        ...
        if not peer_details_list:
            ...
            self._financial_data_cache['competitor_analysis'] = {"summary": competitor_analysis_summary,
                                                                 "peers_data": []}
            return competitor_analysis_summary

        company_name_for_prompt = self.stock_db_entry.company_name or self.ticker
        k_summaries = self._financial_data_cache.get('10k_summaries', {})
        ...
        comp_prompt = (
            f"{prompt_context}\n\nInstruction: Based on the business summary of {company_name_for_prompt} and the list of its competitors with their financial metrics, "
            ...
            f"Highlight any key differences in scale (market cap) or valuation (P/E, P/S) that stand out. "
            f"Address the intensity of competition. Do not invent information not present. If competitor data is sparse, acknowledge that. This summary should complement, not merely repeat, the 10-K business description.")

        comp_summary_ai = self.gemini.generate_text(comp_prompt);
        time.sleep(3)
        if comp_summary_ai and not comp_summary_ai.startswith("Error:"):
            competitor_analysis_summary = comp_summary_ai
        else:
            competitor_analysis_summary = "AI synthesis of competitor data failed. Basic peer data might be available in snapshot."
            self.data_quality_warnings.append("Competitor analysis AI synthesis failed.")
    ...
    def _parse_ai_investment_thesis_response(self, ai_response_text):
        ...
        if not ai_response_text or ai_response_text.startswith("Error:"):
            ...
            # Populate error message into relevant fields
        ...
        # Regex patterns to find sections. More robust to handle variations.
        # Looks for "Heading:" then captures everything until the next known heading or end of string.
        patterns = {
        ...
        for key, pattern in patterns.items():
            ...
            if match:
                ...
                if content:
                    # For single-line fields, take the first line. For multi-line, take all.
                ...
                else:  # Match found but content is empty
                    parsed_data[key] = f"'{key.replace('_', ' ').title()}:' section found but content empty."

        # If no sections were parsed but AI response exists, put the whole response in the thesis.
        if not found_any_section and not ai_response_text.startswith("Error:"):
            logger.warning(
                f"Could not parse distinct sections from AI thesis response for {self.ticker}. Full response will be in 'investment_thesis_full'.")
            parsed_data["investment_thesis_full"] = text_content
            # Other fields remain as default "Review AI Output" or "Not Specified"
    ...
    def _determine_investment_thesis(self):
        ...
        competitor_analysis = self._financial_data_cache.get('competitor_analysis', {}).get("summary", "N/A")
        ...
        metrics_for_prompt = {  # Select key metrics for the prompt
            "P/E Ratio": metrics.get("pe_ratio"), "P/B Ratio": metrics.get("pb_ratio"),
            "P/S Ratio": metrics.get("ps_ratio"),
            "Dividend Yield": metrics.get("dividend_yield"), "ROE": metrics.get("roe"), "ROIC": metrics.get("roic"),
            ...
            "Latest Quarterly Revenue (Source: {src})".format(
                src=metrics.get("key_metrics_snapshot", {}).get("q_revenue_source", "N/A")): metrics.get(
            ...
            if val is not None:
                if isinstance(val, float):
                    if any(kw in name.lower() for kw in ["yield", "growth", "margin", "roe", "roic"]):
                        val_str = f"{val:.2%}"
                    elif "revenue" in name.lower() and "growth" not in name.lower():  # For absolute revenue
                        val_str = f"{val:,.0f}"
                    else:
                        val_str = f"{val:.2f}"
                else:
        ...
        # Add DCF sensitivity summary if available
        if dcf_results.get("dcf_assumptions", {}).get("sensitivity_analysis"):
            prompt += "- DCF Sensitivity Highlights:\n"
            for sens_item in dcf_results["dcf_assumptions"]["sensitivity_analysis"][:2]:  # Show first 2 for brevity
                prompt += f"  - {sens_item['scenario']}: IV {sens_item['intrinsic_value']:.2f} (Upside: {sens_item['upside']:.2% if sens_item['upside'] is not None else 'N/A'})\n"
        ...
        qual_for_prompt = {
            ...
            "Competitive Landscape": competitor_analysis,  # Use the new competitor analysis
        ...
        for name, text_val in qual_for_prompt.items():
            if text_val and isinstance(text_val, str) and not text_val.startswith(
                    "AI analysis") and not text_val.startswith("Section not found") and not text_val.startswith(
                    "Insufficient input"):
                prompt += f"- {name}:\n{text_val[:500].replace('...', '').strip()}...\n\n"  # Truncate for prompt, add more context
            elif text_val:  # Even if it's an error message or "N/A", include it for AI awareness
                prompt += f"- {name}: {text_val}\n\n"

        # Data Quality Warnings to inform AI
        if self.data_quality_warnings:
            prompt += "IMPORTANT DATA QUALITY CONSIDERATIONS:\n"
            for warn_idx, warning_text in enumerate(self.data_quality_warnings):
                prompt += f"- WARNING {warn_idx + 1}: {warning_text}\n"
        ...
        prompt += (
            ...
            "provide a detailed financial analysis and investment thesis. Structure your response *EXACTLY* as follows, using these specific headings on separate lines:\n\n"
            ...
            "Cover: Valuation (DCF, comparables if any), Financial Health & Profitability, Growth Prospects (Revenue & EPS), Economic Moat, Competitive Position, Key Risks (including data quality issues if significant), Management & Strategy (if inferable).]\n"
        )

        ai_response_text = self.gemini.generate_text(prompt)
        parsed_thesis_data = self._parse_ai_investment_thesis_response(ai_response_text)

        # Adjust AI confidence if critical data warnings exist
        if any("CRITICAL:" in warn for warn in self.data_quality_warnings) or \
                any("DATA QUALITY WARNING:" in warn for warn in self.data_quality_warnings and "revenue" in warn):
            if parsed_thesis_data.get("confidence_level", "").lower() == "high":
                logger.warning(
                    f"Downgrading AI confidence from High to Medium for {self.ticker} due to critical data quality warnings.")
                parsed_thesis_data["confidence_level"] = "Medium"
            elif parsed_thesis_data.get("confidence_level", "").lower() == "medium":
                logger.warning(
                    f"Downgrading AI confidence from Medium to Low for {self.ticker} due to critical data quality warnings.")
                parsed_thesis_data["confidence_level"] = "Low"

        logger.info(
            f"Generated thesis for {self.ticker}. Decision: {parsed_thesis_data.get('investment_decision')}, "
    ...
    def analyze(self):
        ...
        final_data_for_db = {}
        try:
            if not self.stock_db_entry:  # Should have been caught by init, but double check
                logger.error(f"Stock DB entry for {self.ticker} not initialized properly. Aborting analysis.")
                return None

            # Ensure the stock_db_entry is bound to the current session, especially if it was created/updated
            self._ensure_stock_db_entry_is_bound()

            # --- Data Fetching Phase ---
            self._fetch_financial_statements()  # Includes FMP, Finnhub, AV
            self._fetch_key_metrics_and_profile_data()  # FMP, Finnhub

            # --- Calculation Phase ---
            final_data_for_db.update(self._calculate_derived_metrics())  # Includes metric snapshot for q_revenue
            ...
            final_data_for_db.update(qual_summaries)
            ...
            # Map data to StockAnalysis model fields
            ...
            for field_name in model_fields:
                if field_name in final_data_for_db:
                    ...
                    # Basic type validation/conversion before setting
                    target_column_type = getattr(StockAnalysis, field_name).type.python_type
                    if target_column_type == float:
                        if isinstance(value_to_set, str):  # Attempt to convert string to float
                            try:
                        ...
                        if isinstance(value_to_set, float) and (math.isnan(value_to_set) or math.isinf(value_to_set)):
                            value_to_set = None
                    elif target_column_type == dict and not isinstance(value_to_set, dict):  # For JSON fields
                        value_to_set = None  # Or attempt json.loads if string
        ...
        except RuntimeError as rt_err:  # Catch init errors if they weren't caught earlier
            logger.critical(f"Runtime error during full analysis for {self.ticker}: {rt_err}", exc_info=True)
            # Session should be handled by the __init__ error path if it failed there
            return None
        except Exception as e:
            ...
            # Ensure session is closed regardless of success or failure of the analyze method
            self._close_session_if_active()

    def _ensure_stock_db_entry_is_bound(self):
        """Ensures the stock_db_entry is associated with the current active session."""
        if not self.stock_db_entry:
            raise RuntimeError(
                f"Stock entry for {self.ticker} is None during binding check. This indicates a prior initialization failure.")

        if not self.db_session.is_active:
            logger.warning(f"DB Session for {self.ticker} was INACTIVE before operation. Re-establishing.")
            self._close_session_if_active()  # Close old one if any
            self.db_session = next(get_db_session())
            # Re-fetch the stock with the new session
            re_fetched_stock = self.db_session.query(Stock).filter(Stock.ticker == self.ticker).first()
            if not re_fetched_stock:
                # This case should ideally not happen if _get_or_create_stock_entry was successful
                raise RuntimeError(
        ...
        # If instance is transient (not yet persisted) or detached, or tied to a different session
        if not instance_state.session or instance_state.session is not self.db_session:
            ...
            current_instance_session_id = id(instance_state.session) if instance_state.session else 'None'
            logger.warning(
                f"Stock {self.ticker} (ID: {obj_id_log}) DETACHED or bound to DIFFERENT session "
                f"(Expected session ID: {id(self.db_session)}, Actual instance session ID: {current_instance_session_id}). Attempting to merge."
            )
            try:
                # Merge the instance into the current session.
                # This reassociates the instance with self.db_session.
                self.stock_db_entry = self.db_session.merge(self.stock_db_entry)
                self.db_session.flush()  # Ensure it's synchronized
                logger.info(
                ...
                logger.error(f"Failed to merge stock {self.ticker} into session: {e_merge}. Re-fetching as a fallback.",
                             exc_info=True)
                # As a robust fallback, re-fetch from DB with the current session
                ...
                if re_fetched_from_db_after_merge_fail:
                    ...
                    # This is a critical state, means the stock somehow disappeared or session is deeply troubled.
                    raise RuntimeError(
...
if __name__ == '__main__':
    ...
    tickers_to_test = ["AAPL", "MSFT", "NKE"]  # Add a problematic one if you have a good test case

    for ticker_symbol in tickers_to_test:
        analysis_result_obj = None
        try:
            logger.info(f"--- Analyzing {ticker_symbol} ---")
            analyzer_instance = StockAnalyzer(ticker=ticker_symbol)  # Analyzer manages its own session
            ...
            if analysis_result_obj and hasattr(analysis_result_obj, 'stock'):
                ...
                logger.info(
                    ...
                    f"Value: {analysis_result_obj.key_metrics_snapshot.get('latest_q_revenue', 'N/A') if analysis_result_obj.key_metrics_snapshot else 'N/A'})"
                    )
                logger.info(
                    f"  P/E: {analysis_result_obj.pe_ratio}, P/B: {analysis_result_obj.pb_ratio}, ROE: {analysis_result_obj.roe}")
                if analysis_result_obj.qualitative_sources_summary:
                    logger.info(
                        f"  10K URL used: {analysis_result_obj.qualitative_sources_summary.get('10k_filing_url_used', 'N/A')}")
                if analysis_result_obj.competitive_landscape_summary:
                    logger.info(
                        f"  Competitive Landscape Summary: {analysis_result_obj.competitive_landscape_summary[:200]}...")
            ...
            logger.error(f"Could not run StockAnalyzer for {ticker_symbol} due to critical init error: {rt_err}")
            ...
            time.sleep(20)  # Stagger API calls if running multiple tests