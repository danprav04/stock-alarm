    ...
    def request(self, method, endpoint, params=None, data=None, json_data=None, use_cache=True,
        ...
        for attempt in range(API_RETRY_ATTEMPTS):
            try:
                ...
                sensitive_header_keys = ["X-RapidAPI-Key", "Authorization", "Token"]
    ...
    def _load_cik_map(self):
        if self._cik_map is None:
            ...
            try:
                ...
                logger.error(f"Error fetching CIK map from SEC: {e}", exc_info=True); self._cik_map = {}
            except json.JSONDecodeError as e_json:
                logger.error(f"Error decoding CIK map JSON from SEC: {e_json}", exc_info=True); self._cik_map = {}
...
class GeminiAPIClient:
    ...
    def generate_text(self, prompt, model="gemini-2.5-flash-preview-05-20"):
        ...
        if total_keys == 0:
            logger.error("Gemini: No API keys configured."); return "Error: No Google API keys."

        # Apply hard truncation limit for the entire prompt as a final safety net.
        # Specific truncation for summaries should be handled by the caller or summarize_text_with_context.
        if len(prompt) > GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE:
            logger.warning(
                f"Gemini prompt length {len(prompt)} exceeds hard limit {GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE}. "
                f"Truncating to {GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE} chars for safety.")
            prompt = prompt[:GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE] + "\n...[PROMPT HARD TRUNCATED DUE TO EXCESSIVE LENGTH]..."
        ...
        for overall_attempt_num in range(total_keys * max_attempts_per_key):
            ...
            payload = {
                ...
                "generationConfig": {
                    "temperature": 0.5, "maxOutputTokens": 8192, "topP": 0.9, "topK": 35
                },
                "safetySettings": [
            ...
            try:
                ...
                if "candidates" in response_json and response_json["candidates"]:
                    ...
                    if "text" in content_part: return content_part["text"]
                    else: logger.error(f"Gemini response missing text for key ...{api_key[-4:]}: {response_json}")
                else: logger.error(f"Gemini response malformed for key ...{api_key[-4:]}: {response_json}")
    ...
    def summarize_text_with_context(self, text_to_summarize, context_summary, desired_output_instruction):
        """
        Generates a summary for the given text, incorporating context and specific instructions.
        Note: This method itself does not do chunking. Chunking should be handled by the caller
        if text_to_summarize is very large. The GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE
        in generate_text() is the final safety net for overall prompt length.

        Args:
            text_to_summarize (str): The primary text content to be summarized.
            context_summary (str): Contextual information to guide the summarization.
            desired_output_instruction (str): Specific instructions on what the summary should focus on or include.
        """
        prompt = (
...
def scrape_article_content(url):
    try:
        ...
        if 'html' not in response.headers.get('content-type', '').lower():
            logger.warning(
                f"Content type for {url} is not HTML ('{response.headers.get('content-type')}'). Skipping scrape."); return None
        soup = BeautifulSoup(response.content, 'lxml')
        for tag_name in ['script', 'style', 'nav', 'header', 'footer', 'aside', 'form', 'iframe', 'noscript', 'link', 'meta']:
        ...
        for selector in selectors:
            ...
            if tag:
                for unwanted_pattern in ['ad', 'social', 'related', 'share', 'comment', 'promo', 'sidebar', 'popup', 'banner', 'meta-info', 'byline-container']:
                    for sub_tag in tag.find_all(lambda t: any(unwanted_pattern in c for c in t.get('class', [])) or \
                        ...
                        sub_tag.decompose()
                main_content_html = tag; break
        article_text = ""
        if main_content_html:
            text_parts = [p.get_text(separator=' ', strip=True) for p in
                          main_content_html.find_all(['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'span', 'div'], recursive=True)
        ...
        if len(article_text) < 150:
            logger.info(f"Extracted text from {url} is very short ({len(article_text)} chars). Might be a stub or paywall.")
...
def extract_S1_text_sections(filing_text, sections_map):
    ...
    if not found_sections:
        ...
        for key, patterns_list in sections_map.items():
            if len(patterns_list) > 1:
                desc_name_pattern = re.compile(r"^\s*" + re.escape(patterns_list[1]) + r"\s*$", re.IGNORECASE | re.MULTILINE)
    ...
    if not found_sections:
        logger.warning("No sections extracted from SEC filing based on any patterns."); return {}
