...
from config import (
    ...
    GEMINI_PROMPT_MAX_CHARS_HARD_TRUNCATE,
    SUMMARIZATION_CHUNK_SIZE_CHARS
)

MAX_IPO_ANALYSIS_WORKERS = 1
...
class IPOAnalyzer:
    ...
    def _fetch_s1_data(self, db_session, ipo_db_entry):
        ...
        if not target_cik:
            if ipo_db_entry.symbol:
                ...
                if target_cik:
                    ...
                    logger.warning(f"No CIK via symbol {ipo_db_entry.symbol} for '{ipo_db_entry.company_name}'.")
                    return None, None
            else:
                logger.warning(f"No CIK/symbol for '{ipo_db_entry.company_name}'. Cannot fetch S-1.")
                return None, None
        ...
        if s1_url:
            if ipo_db_entry.s1_filing_url != s1_url:
                ...
                try:
                    db_session.commit()
                except SQLAlchemyError as e:  # pylint: disable=unused-variable
                    db_session.rollback()
            ...
            if filing_text:
                logger.info(f"Fetched S-1/F-1 text (len: {len(filing_text)}) for {ipo_db_entry.company_name}")
    ...
    def _analyze_single_ipo_task(self, db_session, ipo_data_from_fetch):
        ...
        resp1 = self.gemini.generate_text(prompt1);
        time.sleep(3)
        ...
        resp2 = self.gemini.generate_text(prompt2);
        time.sleep(3)
        analysis_payload["s1_risk_factors_summary"] = self._parse_ai_section(resp2, ["Key Risk Factors",
        ...
        analysis_payload["s1_mda_summary"] = analysis_payload["s1_financial_health_summary"]
        ...
        if analysis_payload.get('s1_business_summary') and "Section not found" not in analysis_payload[
            's1_business_summary']:
            synth_prompt_parts.append(f"Business={analysis_payload['s1_business_summary'][:150]}")
        if analysis_payload.get('s1_risk_factors_summary') and "Section not found" not in analysis_payload[
            's1_risk_factors_summary']:
            synth_prompt_parts.append(f"Risks={analysis_payload['s1_risk_factors_summary'][:150]}")
        if analysis_payload.get('s1_financial_health_summary') and "Section not found" not in analysis_payload[
            's1_financial_health_summary']:
            synth_prompt_parts.append(f"Financials={analysis_payload['s1_financial_health_summary'][:150]}")
        synth_prompt_parts.append(
            "Provide: Investment Stance (e.g., Monitor, Attractive Risk/Reward, Avoid), Reasoning, Critical Verification Points from S-1.")
        synth_prompt = " ".join(synth_prompt_parts)

        gemini_synth = self.gemini.generate_text(synth_prompt);
        time.sleep(3)
    ...
    def _parse_ai_section(self, ai_text, section_header_keywords):
        ...
        capture = False
        content = []
        all_headers = ["business model:", "competitive landscape:", "industry outlook:",
                       "significant risk factors:", "key risk factors:", "risk factors summary:",
                       "use of ipo proceeds:", "financial health from md&a:", "financial health summary:",
        ...
        for line in lines:
            ...
            if matched_kw:
                ...
                continue
            if capture:
                if any(norm_line.startswith(h) for h in all_headers if h not in keywords):
                    break
                content.append(line)
    ...
    def _parse_ai_synthesis(self, ai_response):
        ...
        if ai_response.startswith("Error:") or not ai_response:
            ...
            parsed["reasoning"] = ai_response if ai_response else "AI Error: Empty response."
            return parsed
        ...
        if parsed["investment_decision"].startswith("Section not found"):
            parsed["investment_decision"] = "Review AI Output"
        if parsed["reasoning"].startswith("Section not found"):
